...
class CommandLine():
    ...
    def run(self):
        ...
        while True:
            ...
            if mode == 'milvus':
                ...
                print('  1.使用`build data/Chinese_law/criminal_general_provisions.txt`来进行知识库构建。')
                print('  2.已有索引可以使用`ask`进行提问, `-d`参数以debug模式进入。')
                print('  3.删除已有索引可以使用`remove criminal_general_provisions.txt`。')
                self._mode = 'milvus'
                break
            elif mode == 'pipeline':
    ...
    def parse_input(self, text):
        ...
        if commands[0] == 'build':
            ...
            print('(rag) 只有[build|ask|remove|eval|quit]中的操作, 请重新尝试。')
